...
server_cp = ConfigParser.SafeConfigParser()
server_cp.read(os.path.dirname(os.path.realpath(__file__)) + '/ngrok-server.conf')

host = server_cp.get('server', 'host')
port = int(server_cp.get('server', 'port'))
bufsize = int(server_cp.get('server', 'bufsize'))
usecert = server_cp.get('server', 'usecert')

tunnels_cp = ConfigParser.SafeConfigParser()
tunnels_cp.read(os.path.dirname(os.path.realpath(__file__)) + '/ngrok-tunnels.conf')
tunnels = tunnels_cp.sections()

Tunnels = list()

for tunnel in tunnels:
   tunnel_dict = dict()
   tunnel_options = tunnels_cp.options(tunnel)
   for option in tunnel_options:
       if option == 'rport' or option == 'lport':
           tunnel_dict[option] = int(tunnels_cp.get(tunnel, option))
       else:
           tunnel_dict[option] = tunnels_cp.get(tunnel, option)

   Tunnels.append(tunnel_dict)
...
def connectremote(host, port):
    ...
    try:
        ...
        if usecert == 'true' or usecert == '1':
            certpath = os.path.dirname(os.path.realpath(__file__)) + "/cert/ngrok-server.crt"
            ssl_client = ssl.wrap_socket(client, ca_certs=certpath , cert_reqs=ssl.CERT_REQUIRED)
        else:
